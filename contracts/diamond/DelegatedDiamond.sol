...
contract DelegatedDiamond {
    ...
    fallback() external payable {
        LibDelegatedDiamond.DelegatedDiamondStorage storage ds;
        bytes32 position = LibDelegatedDiamond.DIAMOND_STORAGE_POSITION;
        // get diamond storage
        assembly {
            ds.slot := position
        }
        // get facet from function selector
        address facet = IFacetProvider(ds.facetProvider).selectorToFacetAndPosition(msg.sig).facetAddress;
