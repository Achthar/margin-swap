 ...
 * @author Achthar
 */
contract MarginTraderFacet is IUniswapV3SwapCallback, WithStorage {
    ...
    function swapBorrowExactIn(uint256 _protocolId, ExactInputSingleParams memory _uniswapV3params) external payable onlyOwner returns (uint256) {
        address pool = IDataProvider(ps().dataProvider).getV3Pool(_uniswapV3params.tokenIn, _uniswapV3params.tokenOut, _uniswapV3params.fee);
        ...
        returns (uint256 amountIn)
    {
        ...
        CErc20Interface cTokenOut = IDataProvider(ps().dataProvider).cToken(_marginSwapParams.tokenOut, _protocolId);
        cTokenOut.mint(_marginSwapParams.userAmountProvided);

        address pool = IDataProvider(ps().dataProvider).getV3Pool(_marginSwapParams.tokenIn, _marginSwapParams.tokenOut, _marginSwapParams.fee);
        SwapCallbackData memory data = SwapCallbackData({
    ...
    function uniswapV3SwapCallback(
        ...
        (CErc20Interface cTokenIn, CErc20Interface cTokenOut) = IDataProvider(ps().dataProvider).validatePoolAndFetchCTokens(
            msg.sender,
            tokenIn,
            tokenOut,
