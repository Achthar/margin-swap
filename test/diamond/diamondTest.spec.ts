...
describe('DiamondTest', async function () {
  ...
  it('should add test1 functions', async () => {
    ...
    const selectors = remove(getSelectors(test1Facet), ['supportsInterface(bytes4)'])
    tx = await diamondCutFacet.connect(signer).diamondCut(
      [{
  ...
  it('should replace supportsInterface function', async () => {
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const selectors = get(getSelectors(Test1Facet), ['supportsInterface(bytes4)'])
  ...
  it('should remove some test2 functions', async () => {
    ...
    const selectors = remove(getSelectors(test2Facet), functionsToKeep)
    tx = await diamondCutFacet.connect(signer).diamondCut(
      [{
    ...
    assert.sameMembers(result, get(getSelectors(test2Facet), functionsToKeep))
  })

  it('should remove some test1 functions', async () => {
    const test1Facet = await ethers.getContractAt('Test1Facet', diamondAddress)
    const functionsToKeep = ['test1Func2()', 'test1Func11()', 'test1Func12()']
    const selectors = remove(getSelectors(test1Facet), functionsToKeep)
    tx = await diamondCutFacet.connect(signer).diamondCut(
      [{
        facetAddress: ethers.constants.AddressZero,
        action: FacetCutAction.Remove,
    ...
    assert.sameMembers(result, get(getSelectors(test1Facet), functionsToKeep))
  })
  ...
  it('add most functions and facets', async () => {
    const diamondLoupeFacetSelectors = remove(getSelectors(diamondLoupeFacet), ['supportsInterface(bytes4)'])
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const Test2Facet = await ethers.getContractFactory('Test2Facet')
    ...
    const cut = [
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: remove(diamondLoupeFacetSelectors, ['facets()'])
      },
      {
    ...
    tx = await diamondCutFacet.connect(signer).diamondCut(cut, ethers.constants.AddressZero, '0x', { gasLimit: 8000000 })
    receipt = await tx.wait()
    if (!receipt.status) {
      throw Error(`Diamond upgrade failed: ${tx.hash}`)
    }
