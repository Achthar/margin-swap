...
import { OracleTestPeriphery, TestERC20Periphery } from '../../../types'
import { expandTo18Decimals } from './shared/expandTo18Decimals'
import snapshotGasCost from './shared/snapshotGasCost'

describe('OracleLibrary', () => {
  let loadFixture: ReturnType<typeof waffle.createFixtureLoader>
  let tokens: TestERC20Periphery[]
  let oracle: OracleTestPeriphery
  ...
  const oracleTestFixture = async () => {
    ...
    const oracleFactory = await ethers.getContractFactory('OracleTestPeriphery')
    const oracle = await oracleFactory.deploy()

    return {
      tokens: tokens as TestERC20Periphery[],
      oracle: oracle as OracleTestPeriphery,
    }
  }
